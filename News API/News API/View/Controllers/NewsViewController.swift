...
protocol NewsViewControllerProtocol: AnyObject {
  var viewModel: NewsViewModelProtocol { get set }
}
...
final class NewsViewController: UIViewController,NewsViewControllerProtocol {
  ...
  private func configureNavigationController() {
    navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(systemName: Constant.leftBarButtonItemImage), style: .done, target: self, action: #selector(openSearch))
...
extension NewsViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    ...
    headerLabel.text = Constant.headerLabel
    return headerView
  }
  ...
  func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt
    ...
    let addActionNews = UIContextualAction(style: .normal, title: Constant.addActionNews) { [weak self]  _,_,
      completion in
      ...
      self?.viewModel.saveNews(News: saveNews)
      self?.displayNotificationAndDismissIn2Second(notificationTitle: Constant.saveTitle,
                                                   notificationMessage: Constant.saveMessage)
      completion(true)
    }
